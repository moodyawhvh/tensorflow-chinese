...
def load_graph(model_file):
  """从 .pb 文件加载冻结图,并导入到一个新的 Graph 中。"""
  graph = tf.Graph()
  graph_def = tf.compat.v1.GraphDef()

  # 以二进制方式读取序列化的 GraphDef
...
def read_tensor_from_image_file(file_name,
                                ...
                                input_mean=0,
                                input_std=255):
  """读取图片,缩放到模型输入尺寸并做归一化,返回可直接喂给模型的张量。"""
...
if __name__ == "__main__":
  # 默认输入:Grace Hopper 示例图片、Inception v3 冻结模型与 ImageNet 标签
